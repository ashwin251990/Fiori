		...
		onTheMasterPage: {
			...
			actions: {
									...
									var sCurrentId,
										aItemsNotInTheList = aEntityData.filter(function (oObject) {
											return !oList.getItems().some(function (oListItem) {
												return oListItem.getBindingContext().getProperty("Tanum") === oObject.Tanum;
											});
										});

									if (!aItemsNotInTheList.length) {
										// Not enough items all of them are displayed so we take the last one
										sCurrentId = aEntityData[aEntityData.length - 1].Tanum;
									} else {
										sCurrentId = aItemsNotInTheList[0].Tanum;
									}
				...
				iRememberTheListItem: function (oListItem) {
					...
					this.getContext().currentItem = {
						...
						id: oBindingContext.getProperty("Tanum"),
						title: oBindingContext.getProperty("Lgnum")
					};
				},
			...
			assertions: {
				...
				theListHeaderDisplaysZeroHits: function () {
					return this.waitFor({
						...
						matchers: new PropertyStrictEquals({
							name: "title",
							value: "TransferOrderSet (0)"
						}),
						success: function () {
							Opa5.assert.ok(true, "The list header displays 'TransferOrderSet (0)'");
						},
				...
				theListShouldBeSortedAscendingOnName: function () {
					return this.theListShouldBeSortedAscendingOnField("Lgnum");
				},
				...
				theListShouldHaveAllEntries: function () {
					...
					// retrieve all TransferOrderSet to be able to check for the total amount
					...
					return this.waitFor({
						...
						success: function (oList) {
							...
							this.waitFor({
								...
								matchers: new PropertyStrictEquals({
									name: "title",
									value: "TransferOrderSet (" + iExpectedLength + ")"
								}),
								success: function () {
